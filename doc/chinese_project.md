...
| **认证方式** | 无需登录认证                  |
| **数据存储** | SQLite + 文件系统（模型权重）     |
| **网络绑定** | API默认绑定127.0.0.1（仅本地访问） |
| **隐私保护** | 数据完全本地化，用户自主掌控          |
...
| -------- | -------------------------- |
| **多数据集** | 支持上传和管理多个用电数据集             |
| **多会话**  | 支持多个独立的LLM对话会话，每个会话关联特定数据集 |
| **全局告警** | 单一告警配置，应用于模拟数据实时检测         |
...
- **技术实现**：
  ...
  - **方案B（推荐）**：K-Means聚类算法 + 人工语义标注
    - 提取用电特征（24小时平均模式、峰值时段、日均用电量等）
    - 使用K-Means算法自动聚类（k=3或5）
    - 分析聚类中心特征，赋予语义标签
...
- **数据集**：
  ...
  - 标签生成：基于K-means聚类算法自动标注（无监督学习）
    
    - 阶段1：使用K-means对25栋建筑的用电特征进行聚类（聚类数=5）
    - 阶段2：分析每个聚类中心的24小时用电曲线，人工赋予语义标签
    - 阶段3：使用聚类标签作为监督信号训练LSTM模型
...
| 组件        | 技术选型                                                               | 理由            |
| --------- | ------------------------------------------------------------------ | ------------- |
| **Web框架** | Robyn 0.72.2+（高性能异步框架）                                             | 任务书要求，前后端分离架构 |
| **深度学习**  | PyTorch 2.8.0 + torchvision 0.23.0 + torchaudio 2.8.0 (CUDA 12.9)  | 必须，支持CUDA     |
| **数据处理**  | Pandas 2.3.3+, NumPy 2.3.5+                                        | 导师要求          |
| **可视化**   | Matplotlib 3.10.8+, Seaborn 0.13.2+, Pyecharts 2.0.9+              | 导师要求          |
| **机器学习**  | scikit-learn 1.8.0+                                                | 聚类与特征工程       |
| **LLM集成** | LangChain 1.2.0+, OpenAI 2.14.0+, Anthropic 0.75.0+, Ollama 0.6.1+ | 多后端支持         |
| **任务调度**  | APScheduler 3.11.2+                                                | 定时任务与数据生成     |
| **文档处理**  | python-docx 1.2.0+, openpyxl 3.1.5+, WeasyPrint 67.0+              | 报告导出          |
| **数据存储**  | SQLite（Python内置）                                                   | 单文件数据库，零配置部署  |
| **包管理**   | uv                                                                 | 现代化包管理        |
...
| 组件          | 技术选型                                          | 理由           |
| ----------- | --------------------------------------------- | ------------ |
| **框架**      | React 18.3.1 + TypeScript 5.9.3 + Vite 5.4.21 | 任务书要求，展示全栈能力 |
| **图表库**     | ECharts 6.0.0 + echarts-for-react 3.0.5       | 交互式可视化       |
| **UI框架**    | Ant Design 6.1.2                              | 现代化UI        |
| **HTTP客户端** | Axios 1.13.2                                  | 数据请求         |
...
│                     数据存储层                            │
│  ┌──────────────────────────────────────────────────┐  │
│  │  SQLite 数据库 (energy.db, WAL模式)               │  │
│  │  - 用电数据 / 对话记录 / 分析缓存 / 元数据          │  │
│  └──────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────┐  │
│  │  文件系统: 模型权重(.pth) / 导出报告(PDF/Excel)    │  │
│  └──────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
                            ↑
┌─────────────────────────────────────────────────────────┐
